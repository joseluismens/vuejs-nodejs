...
export default class ConversionController {
    ...
    static convertirUF = async (req: Request, res: Response) => {
        ...
        const date = await this.convertirFecha(fecha);
                ...
                https.get(`https://mindicador.cl/api/uf/${date}`, (resolve:any ) => {
                    ...
                    resolve.on('end', async () => {
                        try {
                            ...
                            const uf = indicator.serie[0].valor;
                            const conversion =  Math.ceil(uf * valor);

                           const create = await Conversion.save({
    ...
    static historial = async (req:Request, res:Response)=>{
        ...
        try {
            let data:any = [];
            const historiales = await Conversion.find({order:{
                created_at:"DESC"},relations:["usuario"]});
            if (historiales){
                data = historiales;
            }
    ...
    static conversiones = async (req:Request, res:Response)=>{
        ...
        try {
            let data:any = [];
            const historales = await Conversion.find({where:{usuario_id:id},order:{created_at:'DESC'}});
            if (historales){
                data = historales;
            }
            return res.status(200).json({data});
